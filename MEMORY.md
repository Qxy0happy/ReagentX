...
- **HTTPS**：后端默认监听 `:8081`（HTTP），通过 Caddy 在 `:8080` 提供 HTTPS
  - Windows 本地开发：`caddy reverse-proxy --from localhost:8080 --to localhost:8081`（Caddyfile 模式在 Windows SChannel 下有兼容问题）
  - Docker：使用 Caddyfile + tls internal，容器内无 SChannel 问题
- **启动脚本**：`start-backend.ps1`（Windows） / `termux-start.sh`（Termux） / `docker compose up -d`（Docker）
